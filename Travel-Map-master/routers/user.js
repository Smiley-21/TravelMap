...
router.post('/login',async(req,res)=>
{
    ...
    try{
         ...
         if(!user_data)  res.status(500).json({"message":"user or password not found!"});
         const pas = user_data.password;
         if(!req.body.email||!req.body.email!=user_data.email) res.status(500).json({"mess":"email is not correct"});
         else
         {
           const matched = await bcrypt.compare(req.body.password,user_data.password);
           if(matched) res.status(200).json({"_id":user_data._id,"name":user_data.name});
           else res.status(500).json({"message":"password is incorrect"});
         }
